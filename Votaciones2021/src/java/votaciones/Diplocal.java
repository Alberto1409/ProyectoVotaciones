 ...
 * @author PC
 */
@WebServlet(urlPatterns = {"/Diplocal"})
public class Diplocal extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Diplocal</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Diplocal at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        request.setAttribute("diplocal", null);
        request.setAttribute("diplocalE", null);
        
        //presidenteasfd
        conexion con = new conexion();
        Statement stmt = null;

        int fkusuario, pan, prd, pri, verdeeco, pt, movciudadano, morena, pes, rsp, fuerzamex, hagamos, somos, futuro, indep1, nulo;
        ...
        try {
            ...
            stmt.executeUpdate("INSERT INTO diplocal(fkUsuario,pan,prd,pri,verdeecologista,pt,movimientociudadano,"
                    + "morena,pes,rsp,fuerzamexico,hagamos,somos,futuro,luisangelmorales,nulo) VALUES ('" + fkusuario
                    + "','" + pan + "','" + prd + "','" + pri + "','" + verdeeco + "','" + pt + "','" + movciudadano + "','" + morena + "','" + pes
                    + "','" + rsp + "','" + fuerzamex + "','" + hagamos + "','" + somos + "','" + futuro + "','" + indep1 + "','" + nulo + "')");
            request.setAttribute("diplocal", "Registro de votos Exitoso ");
            rd = request.getRequestDispatcher("diplocal.jsp");
            rd.forward(request, response);
        } catch (SQLException ex) {
            Logger.getLogger(Registro.class.getName()).log(Level.SEVERE, null, ex);
            request.setAttribute("diplocalE", "Error en el Registro");
            rd = request.getRequestDispatcher("diplocal.jsp");
            rd.forward(request, response);
        }
        try {
